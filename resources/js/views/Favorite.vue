            ...
            <div class="row-head row no-gutters bg-primary-light text-white">
                <div class="col-lg-6">
                    <div class="panel-block__head bg-wrap BackBlueHeder">
                        <div class="panel-block__head-title">{{$t('favorite')}}</div>
                    </div>
                </div>
...
<script>
    ...
    export default {
        ...
        components: {BannerBotom,BannerTop,ItemsLoop},
        computed: {
            ...mapGetters({
                showMapYesNoSidebar: "map/showMapYesNoSidebar",
                search_firms: "auth/user_favorite"
            })
        },
        created(){
            this.$store.dispatch("auth/get_favorite")
        },
